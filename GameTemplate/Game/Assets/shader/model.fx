...
// ------------------------------
// Toon / Outline shader entry points
// ------------------------------
float3 QuantizeToon(float3 color, int bands)
{
    float b = max(1.0f, (float)bands);
    return floor(saturate(color) * b) / b;
}

float4 PSMainToon(SPSIn In) : SV_Target0
{
    float4 lit = PSMainCore(In, true);
    // 4-step toon shading
    lit.rgb = QuantizeToon(lit.rgb, 4);
    return lit;
}

SPSIn VSMainOutline(SVSIn vsIn)
{
    SPSIn psIn = VSMainCore(vsIn, mWorld, false);

    float3 normalWS = normalize(psIn.normal);
    float4 outlinePosWS = float4(psIn.worldPos + normalWS * 0.015f, 1.0f);

    psIn.pos = mul(mView, outlinePosWS);
    psIn.pos = mul(mProj, psIn.pos);
    psIn.worldPos = outlinePosWS.xyz;
    return psIn;
}

float4 PSMainOutline(SPSIn In) : SV_Target0
{
